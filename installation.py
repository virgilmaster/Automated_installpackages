...
def check_packages(packages_install):
    ...
    if (operation_system == 'Windows'):
        os.system('pip list | findstr ' + packages_install + '> ' + ' checkversion.txt')
        username = os.getlogin()
        print('Dear guests your operation system is: ' + operation_system + ' The ' + packages_install + ' in you laptop is version:')

    elif (operation_system == 'Linux'):
        os.system('pip list | grep ' + packages_install + '> ' + ' checkversion.txt')
    ...
    # 判断安装包是否存在
    while (packages_result).split(' ')[0] == packages_install:
...
def install_packages(packages_install,version_result):
